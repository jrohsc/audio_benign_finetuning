...
This script is based on the original finetune.py from Qwen-Audio,
adapted for the filtered VoiceBench finetuning pipeline.

Usage:
    python 2_finetune_qwen.py \
        --model_name_or_path Qwen/Qwen-Audio-Chat \
        --data_path data/qwen_filtered_voicebench/voicebench_qwen_benign_0.5.jsonl \
        --output_dir checkpoints/qwen_voicebench_finetuned

This script:
1. Loads the filtered VoiceBench dataset (JSONL format)
2. Loads Qwen-Audio model
3. Finetunes using LoRA or full finetuning
4. Saves checkpoints
"""
...
class ModelArguments:
    model_name_or_path: Optional[str] = field(default="Qwen/Qwen-Audio-Chat")


@dataclass
class DataArguments:
    data_path: str = field(
        default=None, metadata={"help": "Path to the training data (JSONL format)."}
    )
    eval_data_path: str = field(
        default=None, metadata={"help": "Path to the evaluation data (JSONL format)."}
    )
