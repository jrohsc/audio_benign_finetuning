#!/bin/bash
# Step 0 (Audio-Semantic): Filter HeySQuAD Accents samples by AUDIO SEMANTIC embedding distance
#
# This script filters HeySQuAD Accents samples based on how AF3 "understands" audio content.
# Uses Audio-Flamingo 3's full encoder pipeline (AudioFlamingo3Encoder + MultiModalProjector).
#
# THREE FILTER TYPES AVAILABLE:
#   - audio_acoustic (0_filter_heysquad_accents_audio_acoustic.sh): TRUE acoustic features
#       Uses: Whisper-Large-V3 encoder only
#       Output: data_acoustic_heysquad_accents/
#
#   - audio_semantic (this script): Audio content as understood by model
#       Uses: AF3's AudioFlamingo3Encoder + MultiModalProjector
#       Output: data_heysquad_accents/
#
#   - text_semantic: Text meaning similarity (not implemented for heysquad_accents)
...
CACHE_DIR="data_heysquad_accents/embedding_cache_af3"
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo ""
            echo "Filter HeySQuAD Accents by AUDIO SEMANTIC embedding similarity to harmful audio."
            echo "Uses AF3's full encoder pipeline (AudioFlamingo3Encoder + MultiModalProjector)."
            echo "Results saved to data_heysquad_accents/ folder."
            echo ""
...
if [ -n "$NUM_SAMPLES" ]; then
    OUTPUT_JSON="data_heysquad_accents/heysquad_accents_audio_semantic_n${NUM_SAMPLES}.json"
    MODE_DESC="num_samples=${NUM_SAMPLES}"
    FILTER_ARGS="--num_samples $NUM_SAMPLES"
elif [ -n "$PERCENTAGE" ]; then
    OUTPUT_JSON="data_heysquad_accents/heysquad_accents_audio_semantic_percentage_${PERCENTAGE}.json"
    MODE_DESC="percentage=${PERCENTAGE}%"
    FILTER_ARGS="--percentage $PERCENTAGE"
elif [ -n "$THRESHOLD" ]; then
    OUTPUT_JSON="data_heysquad_accents/heysquad_accents_audio_semantic_threshold_${THRESHOLD}.json"
...
echo "============================================"
echo "HeySQuAD Accents AUDIO-SEMANTIC Filtering"
echo "============================================"
...
echo "HeySQuAD Accents:  $HEYSQUAD_JSON"
...
echo "============================================"
echo ""
echo "Next step: Run 1_prepare_filtered_dataset.sh with --benign_dataset heysquad_accents --filter_type audio_semantic"
echo "  bash 1_prepare_filtered_dataset.sh --benign_dataset heysquad_accents --filter_type audio_semantic --percentage $PERCENTAGE"
