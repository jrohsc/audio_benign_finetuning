#!/usr/bin/env python3
"""
Filter HeySQuAD samples to find those CLOSEST in embedding space to AdvBench samples.
Uses Audio Flamingo 3's pretrained Whisper encoder or original Whisper to compute audio embeddings.

This is based on 0_filter_closest_to_advbench.py but adapted for HeySQuAD dataset.
"""
    ...
    with torch.no_grad():
        for audio_path in tqdm(audio_files, desc="Computing embeddings"):
            try:
                ...
                # Match dtype with model (handles float16/float32 mismatch)
...
def filter_heysquad_samples(
    ...
    metric: str = "cosine",
    cache_dir: str = None,
    dataset_name: str = "HeySQuAD",
    select_safest: bool = False,
):
    """
    Filter benign samples to find those CLOSEST to AdvBench samples.
    Uses original Whisper-Large-V3 encoder (TRUE ACOUSTIC features).
    """
    # Load benign data
    print(f"Loading {dataset_name} data from {heysquad_json}")
    with open(heysquad_json, 'r') as f:
        heysquad_data = json.load(f)

    # Get audio file paths
    json_dir = Path(heysquad_json).parent
    heysquad_audio_files = []
    for sample in heysquad_data:
        ...
        heysquad_audio_files.append(audio_path)

    print(f"Found {len(heysquad_audio_files)} {dataset_name} samples")
    if heysquad_audio_files:
        print(f"First audio path: {heysquad_audio_files[0]}")
    ...
    if cache_dir:
        ...
        ss_cache = str(cache_dir / "heysquad_embeddings_whisper.npz")
        ab_cache = str(cache_dir / "advbench_en_embeddings_whisper.npz")

    # Compute embeddings using original Whisper
    print(f"\n=== Computing {dataset_name} embeddings using original Whisper encoder ===")
    heysquad_embeddings = compute_whisper_embeddings_original(
        heysquad_audio_files,
        cache_path=ss_cache
    )
    ...
    min_distances, all_distances = compute_distances(heysquad_embeddings, advbench_embeddings, metric)

    # Print statistics
    print(f"\nDistance statistics ({dataset_name} to AdvBench):")
    ...
    if num_samples is not None:
        ...
        effective_top_k = int(len(heysquad_data) * percentage / 100)
    ...
    if effective_top_k is not None:
        ...
        if select_safest:
            ...
            filtered_indices = np.where(min_distances <= threshold)[0]
            print(f"\n=== Filtering with threshold <= {threshold} ===")
        print(f"  Kept {len(filtered_indices)} / {len(heysquad_data)} samples ({100*len(filtered_indices)/len(heysquad_data):.1f}%)")
    ...
    filtered_data = [heysquad_data[i] for i in filtered_indices]
    ...
    # Use dataset_name to create appropriate analysis filename
    analysis_filename = f"{dataset_name.lower()}_closest_to_advbench_analysis.npz"
    base_analysis_path = output_path.parent / analysis_filename
    np.savez(
        base_analysis_path,
        min_distances=min_distances,
        all_distances=all_distances,
        filtered_indices=filtered_indices,
        heysquad_embeddings=heysquad_embeddings,
        advbench_embeddings=advbench_embeddings,
        voicebench_files=heysquad_audio_files,  # Keep name for compatibility
        advbench_files=advbench_audio_files
    )
    print(f"  Distance analysis: {base_analysis_path}")

    # Print some examples
    top10_label = f"Top 10 safest (furthest) {dataset_name} samples from AdvBench" if select_safest else f"Top 10 closest {dataset_name} samples to AdvBench"
    print(f"\n=== {top10_label} ===")
...
def main():
    parser = argparse.ArgumentParser(description="Filter HeySQuAD samples closest to AdvBench (acoustic)")
    parser.add_argument("--heysquad_json", type=str,
                       default="data/heysquad/heysquad_full.json",
                       help="Path to HeySQuAD samples JSON file")
    ...
    parser.add_argument("--output_json", type=str,
                       default="data/heysquad/heysquad_closest_to_advbench.json",
    ...
    parser.add_argument("--cache_dir", type=str, default="data/heysquad/embedding_cache",
                       help="Directory to cache embeddings")
    parser.add_argument("--dataset_name", type=str, default="HeySQuAD",
    ...
    filter_heysquad_samples(
        heysquad_json=args.heysquad_json,
