...
# Results saved to data_acoustic_gammacorpus_usa/ folder.
...
CACHE_DIR="data_acoustic_gammacorpus_usa/embedding_cache"
...
if [ -n "$NUM_SAMPLES" ]; then
    OUTPUT_JSON="data_acoustic_gammacorpus_usa/gammacorpus_usa_acoustic_n${NUM_SAMPLES}.json"
    MODE_DESC="num_samples=${NUM_SAMPLES}"
    FILTER_ARGS="--num_samples $NUM_SAMPLES"
elif [ -n "$PERCENTAGE" ]; then
    OUTPUT_JSON="data_acoustic_gammacorpus_usa/gammacorpus_usa_acoustic_percentage_${PERCENTAGE}.json"
    MODE_DESC="percentage=${PERCENTAGE}%"
    FILTER_ARGS="--percentage $PERCENTAGE"
elif [ -n "$THRESHOLD" ]; then
    OUTPUT_JSON="data_acoustic_gammacorpus_usa/gammacorpus_usa_acoustic_threshold_${THRESHOLD}.json"
