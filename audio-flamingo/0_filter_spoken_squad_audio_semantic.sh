#!/bin/bash
# Step 0 (Audio-Semantic): Filter Spoken-SQuAD samples by AUDIO SEMANTIC embedding distance
#
# This script filters Spoken-SQuAD samples based on how AF3 "understands" audio content.
# Uses Audio-Flamingo 3's full encoder pipeline (AudioFlamingo3Encoder + MultiModalProjector).
#
# THREE FILTER TYPES AVAILABLE:
#   - audio_acoustic (0_filter_spoken_squad_audio_acoustic.sh): TRUE acoustic features
#       Uses: Whisper-Large-V3 encoder only
#       Output: data_acoustic/spoken_squad/
#
#   - audio_semantic (this script): Audio content as understood by model
#       Uses: AF3's AudioFlamingo3Encoder + MultiModalProjector
#       Output: data/spoken_squad/
#
#   - text_semantic (0_filter_spoken_squad_text_semantic.sh): Text meaning similarity
#       Uses: sentence-transformers (all-MiniLM-L6-v2)
#       Output: data_semantic/spoken_squad/
...
CACHE_DIR="data/spoken_squad/embedding_cache_af3"
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo ""
            echo "Filter Spoken-SQuAD by AUDIO SEMANTIC embedding similarity to harmful audio."
            echo "Uses AF3's full encoder pipeline (AudioFlamingo3Encoder + MultiModalProjector)."
            echo "Results saved to data/spoken_squad/ folder."
            echo ""
            ...
            echo "  --spoken_squad_json PATH    Spoken-SQuAD JSON file"
...
if [ -n "$NUM_SAMPLES" ]; then
    OUTPUT_JSON="data/spoken_squad/spoken_squad_audio_semantic_n${NUM_SAMPLES}.json"
    MODE_DESC="num_samples=${NUM_SAMPLES}"
    FILTER_ARGS="--num_samples $NUM_SAMPLES"
elif [ -n "$PERCENTAGE" ]; then
    OUTPUT_JSON="data/spoken_squad/spoken_squad_audio_semantic_percentage_${PERCENTAGE}.json"
    MODE_DESC="percentage=${PERCENTAGE}%"
    FILTER_ARGS="--percentage $PERCENTAGE"
elif [ -n "$THRESHOLD" ]; then
    OUTPUT_JSON="data/spoken_squad/spoken_squad_audio_semantic_threshold_${THRESHOLD}.json"
...
echo "============================================"
echo "Spoken-SQuAD AUDIO-SEMANTIC Filtering"
echo "============================================"
...
python 0_filter_spoken_squad_audio_semantic.py \
    --model_path "$MODEL_PATH" \
    --spoken_squad_json "$SPOKEN_SQUAD_JSON" \
...
echo "============================================"
echo ""
echo "Next step: Run 1_prepare_filtered_dataset.sh with --benign_dataset spoken_squad --filter_type audio_semantic"
echo "  bash 1_prepare_filtered_dataset.sh --benign_dataset spoken_squad --filter_type audio_semantic --percentage $PERCENTAGE"
