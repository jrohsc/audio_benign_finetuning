#!/bin/bash
# Step 0 (Audio-Acoustic): Filter LibriSpeech samples by TRUE ACOUSTIC embedding distance
#
# This script filters LibriSpeech samples based on TRUE AUDIO ACOUSTIC characteristics.
...
CACHE_DIR="data_acoustic_librispeech/embedding_cache"
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo ""
            echo "Filter LibriSpeech by TRUE ACOUSTIC embedding similarity to harmful audio."
...
if [ -n "$NUM_SAMPLES" ]; then
    OUTPUT_JSON="data_acoustic_librispeech/librispeech_acoustic_n${NUM_SAMPLES}.json"
    MODE_DESC="num_samples=${NUM_SAMPLES}"
    FILTER_ARGS="--num_samples $NUM_SAMPLES"
elif [ -n "$PERCENTAGE" ]; then
    OUTPUT_JSON="data_acoustic_librispeech/librispeech_acoustic_percentage_${PERCENTAGE}.json"
    MODE_DESC="percentage=${PERCENTAGE}%"
    FILTER_ARGS="--percentage $PERCENTAGE"
elif [ -n "$THRESHOLD" ]; then
    OUTPUT_JSON="data_acoustic_librispeech/librispeech_acoustic_threshold_${THRESHOLD}.json"
...
# Run filtering
python 0_filter_librispeech.py \
