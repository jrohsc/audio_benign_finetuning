#!/usr/bin/env python3
"""
Filter BBH (VoiceBench BIG-Bench Hard) samples to find those CLOSEST
...
def filter_bbh_audio_semantic(
    ...
    """Filter BBH samples by AUDIO-SEMANTIC embedding distance using AF3's encoder."""
    ...
    if cache_dir:
        ...
        bbh_cache = str(cache_dir / "bbh_embeddings_af3.npz")
        ab_cache = str(cache_dir / "advbench_en_embeddings_af3.npz")

    print(f"\n=== Computing BBH embeddings using AF3 encoder ===")
    bbh_embeddings = compute_af3_embeddings(
        bbh_audio_files,
    ...
    base_analysis_path = output_path.parent / "bbh_audio_semantic_analysis.npz"
    np.savez(
        base_analysis_path,
        ...
        bbh_files=bbh_audio_files,
        advbench_files=advbench_audio_files
    )
    print(f"  Distance analysis: {base_analysis_path}")

    top10_label = "safest (furthest) BBH" if select_safest else "closest BBH"
...
def main():
    parser = argparse.ArgumentParser(description="Filter BBH samples by AUDIO-SEMANTIC similarity using AF3")
    ...
    parser.add_argument("--output_json", type=str,
                       default="data/bbh/bbh_audio_semantic.json",
    ...
    parser.add_argument("--cache_dir", type=str, default="data/bbh/embedding_cache_af3",
