...
Compare with:
- 0_filter_spoken_squad.py: TRUE ACOUSTIC features (Whisper-Large-V3 encoder only)
- 0_filter_spoken_squad_semantic.py: TEXT semantic (sentence-transformers)
"""
...
def filter_spoken_squad_audio_semantic(
    ...
    """
    Filter Spoken-SQuAD samples by AUDIO-SEMANTIC embedding distance using AF3's encoder.
    """
    ...
    if cache_dir:
        ...
        ss_cache = str(cache_dir / "spoken_squad_embeddings_af3.npz")
        ab_cache = str(cache_dir / "advbench_en_embeddings_af3.npz")

    # Compute embeddings using AF3 encoder
    print(f"\n=== Computing Spoken-SQuAD embeddings using AF3 encoder ===")
    spoken_squad_embeddings = compute_af3_embeddings(
        spoken_squad_audio_files,
    ...
    base_analysis_path = output_path.parent / "spoken_squad_audio_semantic_analysis.npz"
    np.savez(
        base_analysis_path,
        ...
        voicebench_files=spoken_squad_audio_files,
        advbench_files=advbench_audio_files
    )
    print(f"  Distance analysis: {base_analysis_path}")

    # Print some examples
    top10_label = "safest (furthest) Spoken-SQuAD" if select_safest else "closest Spoken-SQuAD"
...
def main():
    parser = argparse.ArgumentParser(description="Filter Spoken-SQuAD samples by AUDIO-SEMANTIC similarity using AF3")
    ...
    parser.add_argument("--output_json", type=str,
                       default="data/spoken_squad/spoken_squad_audio_semantic.json",
    ...
    parser.add_argument("--cache_dir", type=str, default="data/spoken_squad/embedding_cache_af3",
    ...
    filter_spoken_squad_audio_semantic(
