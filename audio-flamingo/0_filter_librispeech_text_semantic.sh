#!/bin/bash
# Step 0 (Text-Semantic): Filter LibriSpeech samples by TEXT SEMANTIC embedding similarity
#
# This script filters LibriSpeech samples based on TEXT MEANING similarity to harmful prompts.
# Uses sentence-transformers to extract text embeddings from transcriptions.
#
# Output is saved to data_semantic_librispeech/ folder
...
TEXT_COLUMN="goal"
CACHE_DIR="data_semantic_librispeech/embedding_cache"
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo ""
            echo "Filter LibriSpeech by TEXT SEMANTIC embedding similarity to harmful prompts."
            echo "Uses sentence-transformers to compare text meaning (transcription vs harmful text)."
            ...
            echo ""
            echo "Other options:"
            echo "  --librispeech_json PATH   Path to LibriSpeech JSON file"
...
if [ -n "$NUM_SAMPLES" ]; then
    OUTPUT_JSON="data_semantic_librispeech/librispeech_text_semantic_n${NUM_SAMPLES}.json"
    MODE_DESC="num_samples=${NUM_SAMPLES}"
elif [ -n "$PERCENTAGE" ]; then
    OUTPUT_JSON="data_semantic_librispeech/librispeech_text_semantic_percentage_${PERCENTAGE}.json"
    MODE_DESC="percentage=${PERCENTAGE}%"
elif [ -n "$THRESHOLD" ]; then
    OUTPUT_JSON="data_semantic_librispeech/librispeech_text_semantic_threshold_${THRESHOLD}.json"
...
# Build command
CMD="python 0_filter_librispeech_semantic.py"
CMD="$CMD --librispeech_json \"$LIBRISPEECH_JSON\""
