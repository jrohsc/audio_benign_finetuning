#!/bin/bash
# Step 0 (Audio-Acoustic): Filter BBH samples by TRUE ACOUSTIC embedding distance
#
# This script filters BBH (BIG-Bench Hard) samples based on TRUE AUDIO ACOUSTIC characteristics.
...
CACHE_DIR="data_acoustic_bbh/embedding_cache"
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo ""
            echo "Filter BBH by TRUE ACOUSTIC embedding similarity to harmful audio."
            echo "Uses Whisper-Large-V3 encoder ONLY (without AF3's projector)."
            echo "Results saved to data_acoustic_bbh/ folder."
            echo ""
...
if [ -n "$NUM_SAMPLES" ]; then
    OUTPUT_JSON="data_acoustic_bbh/bbh_acoustic_n${NUM_SAMPLES}.json"
    MODE_DESC="num_samples=${NUM_SAMPLES}"
    FILTER_ARGS="--num_samples $NUM_SAMPLES"
elif [ -n "$PERCENTAGE" ]; then
    OUTPUT_JSON="data_acoustic_bbh/bbh_acoustic_percentage_${PERCENTAGE}.json"
    MODE_DESC="percentage=${PERCENTAGE}%"
    FILTER_ARGS="--percentage $PERCENTAGE"
elif [ -n "$THRESHOLD" ]; then
    OUTPUT_JSON="data_acoustic_bbh/bbh_acoustic_threshold_${THRESHOLD}.json"
...
echo "============================================"
echo "BBH TRUE ACOUSTIC Filtering"
echo "============================================"
...
# Run filtering (reuse the generic filter script with bbh paths)
python 0_filter_bbh.py \
    --bbh_json "$BBH_JSON" \
