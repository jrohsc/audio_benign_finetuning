...
Compare with:
- 0_filter_heysquad.py: TRUE ACOUSTIC features (Whisper-Large-V3 encoder only)
- 0_filter_heysquad_semantic.py: TEXT semantic (sentence-transformers)
"""
    ...
    with torch.no_grad():
        for audio_path in tqdm(audio_files, desc="Computing AF3 embeddings"):
            try:
                ...
                # Check if this is AudioFlamingo3Encoder (has avg_pooler)
...
def filter_heysquad_audio_semantic(
    heysquad_json: str,
    ...
    """
    Filter benign samples by AUDIO-SEMANTIC embedding distance using AF3's encoder.
    """
    # Load benign data
    ...
    if cache_dir:
        ...
        ss_cache = str(cache_dir / "heysquad_embeddings_af3.npz")
        ab_cache = str(cache_dir / "advbench_en_embeddings_af3.npz")

    # Compute embeddings using AF3 encoder
    print(f"\n=== Computing {dataset_name} embeddings using AF3 encoder ===")
    heysquad_embeddings = compute_af3_embeddings(
        heysquad_audio_files,
    ...
    if effective_top_k is not None:
        ...
        if select_safest:
            ...
            filtered_indices = np.where(min_distances <= threshold)[0]
            print(f"\n=== Filtering with threshold <= {threshold} ===")
        print(f"  Kept {len(filtered_indices)} / {len(heysquad_data)} samples")
    ...
    # Save distance analysis - use dataset name in filename
    dataset_name_lower = dataset_name.lower().replace(" ", "_")
    base_analysis_path = output_path.parent / f"{dataset_name_lower}_audio_semantic_analysis.npz"
    np.savez(
        base_analysis_path,
        ...
        voicebench_files=heysquad_audio_files,
        advbench_files=advbench_audio_files
    )
    print(f"  Distance analysis: {base_analysis_path}")

    # Print some examples
    top10_label = f"safest (furthest) {dataset_name}" if select_safest else f"closest {dataset_name}"
...
def main():
    parser = argparse.ArgumentParser(description="Filter HeySQuAD samples by AUDIO-SEMANTIC similarity using AF3")
    ...
    parser.add_argument("--output_json", type=str,
                       default="data/heysquad/heysquad_audio_semantic.json",
    ...
    parser.add_argument("--cache_dir", type=str, default="data/heysquad/embedding_cache_af3",
    ...
    filter_heysquad_audio_semantic(
