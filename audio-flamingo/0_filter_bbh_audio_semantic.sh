#!/bin/bash
# Step 0 (Audio-Semantic): Filter BBH samples by AUDIO SEMANTIC embedding distance
#
# This script filters BBH (VoiceBench BIG-Bench Hard) samples based on
...
CACHE_DIR="data/bbh/embedding_cache_af3"
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo ""
            echo "Filter BBH by AUDIO SEMANTIC embedding similarity to harmful audio."
            echo "Uses AF3's full encoder pipeline (AudioFlamingo3Encoder + MultiModalProjector)."
            echo "Results saved to data/bbh/ folder."
            echo ""
            ...
            echo "  --bbh_json PATH             BBH JSON file"
...
if [ -n "$NUM_SAMPLES" ]; then
    OUTPUT_JSON="data/bbh/bbh_audio_semantic_n${NUM_SAMPLES}.json"
    MODE_DESC="num_samples=${NUM_SAMPLES}"
    FILTER_ARGS="--num_samples $NUM_SAMPLES"
elif [ -n "$PERCENTAGE" ]; then
    OUTPUT_JSON="data/bbh/bbh_audio_semantic_percentage_${PERCENTAGE}.json"
    MODE_DESC="percentage=${PERCENTAGE}%"
    FILTER_ARGS="--percentage $PERCENTAGE"
elif [ -n "$THRESHOLD" ]; then
    OUTPUT_JSON="data/bbh/bbh_audio_semantic_threshold_${THRESHOLD}.json"
...
echo "============================================"
echo "BBH AUDIO-SEMANTIC Filtering"
echo "============================================"
...
python 0_filter_bbh_audio_semantic.py \
...
echo "============================================"
echo ""
echo "Next step: Run 1_prepare_filtered_dataset.sh with --benign_dataset bbh --filter_type audio_semantic"
echo "  bash 1_prepare_filtered_dataset.sh --benign_dataset bbh --filter_type audio_semantic --percentage $PERCENTAGE"
