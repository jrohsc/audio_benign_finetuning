...
Compare with:
- 0_filter_librispeech.py: TRUE ACOUSTIC features (Whisper-Large-V3 encoder only)
- 0_filter_librispeech_semantic.py: TEXT semantic (sentence-transformers)
"""
...
def filter_librispeech_audio_semantic(
    ...
    """
    Filter LibriSpeech samples by AUDIO-SEMANTIC embedding distance using AF3's encoder.
    """
    ...
    if cache_dir:
        ...
        ls_cache = str(cache_dir / "librispeech_embeddings_af3.npz")
        ab_cache = str(cache_dir / "advbench_en_embeddings_af3.npz")

    # Compute embeddings using AF3 encoder
    print(f"\n=== Computing LibriSpeech embeddings using AF3 encoder ===")
    librispeech_embeddings = compute_af3_embeddings(
        librispeech_audio_files,
    ...
    base_analysis_path = output_path.parent / "librispeech_audio_semantic_analysis.npz"
    np.savez(
        base_analysis_path,
        ...
        voicebench_files=librispeech_audio_files,
        advbench_files=advbench_audio_files
    )
    print(f"  Distance analysis: {base_analysis_path}")

    # Print some examples
    top10_label = "safest (furthest) LibriSpeech" if select_safest else "closest LibriSpeech"
...
def main():
    parser = argparse.ArgumentParser(description="Filter LibriSpeech samples by AUDIO-SEMANTIC similarity using AF3")
    ...
    parser.add_argument("--output_json", type=str,
                       default="data/librispeech/librispeech_audio_semantic.json",
    ...
    parser.add_argument("--cache_dir", type=str, default="data/librispeech/embedding_cache_af3",
    ...
    filter_librispeech_audio_semantic(
