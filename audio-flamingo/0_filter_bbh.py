#!/usr/bin/env python3
"""
Filter BBH (BIG-Bench Hard) samples to find those CLOSEST in embedding space to AdvBench samples.
Uses Audio Flamingo 3's pretrained Whisper encoder or original Whisper to compute audio embeddings.

This is based on 0_filter_heysquad.py but adapted for BBH dataset.
"""
...
def filter_bbh_samples(
    ...
    """
    Filter BBH samples to find those CLOSEST to AdvBench samples.
    Uses original Whisper-Large-V3 encoder (TRUE ACOUSTIC features).
    """
    # Load BBH data
    print(f"Loading BBH data from {bbh_json}")
    with open(bbh_json, 'r') as f:
        bbh_data = json.load(f)

    # Get audio file paths
    json_dir = Path(bbh_json).parent
    bbh_audio_files = []
    for sample in bbh_data:
        ...
        bbh_audio_files.append(audio_path)

    print(f"Found {len(bbh_audio_files)} BBH samples")
    if bbh_audio_files:
        print(f"First audio path: {bbh_audio_files[0]}")
    ...
    if cache_dir:
        ...
        bbh_cache = str(cache_dir / "bbh_embeddings_whisper.npz")
        ab_cache = str(cache_dir / "advbench_en_embeddings_whisper.npz")

    # Compute embeddings using original Whisper
    print(f"\n=== Computing BBH embeddings using original Whisper encoder ===")
    bbh_embeddings = compute_whisper_embeddings_original(
        bbh_audio_files,
        cache_path=bbh_cache
    )
    ...
    min_distances, all_distances = compute_distances(bbh_embeddings, advbench_embeddings, metric)

    # Print statistics
    print(f"\nDistance statistics (BBH to AdvBench):")
    ...
    if num_samples is not None:
        ...
        effective_top_k = int(len(bbh_data) * percentage / 100)
    ...
    if effective_top_k is not None:
        ...
        if select_safest:
            ...
            filtered_indices = np.where(min_distances <= threshold)[0]
            print(f"\n=== Filtering with threshold <= {threshold} ===")
        print(f"  Kept {len(filtered_indices)} / {len(bbh_data)} samples ({100*len(filtered_indices)/len(bbh_data):.1f}%)")
    ...
    filtered_data = [bbh_data[i] for i in filtered_indices]
    ...
    base_analysis_path = output_path.parent / "bbh_closest_to_advbench_analysis.npz"
    np.savez(
        base_analysis_path,
        min_distances=min_distances,
        all_distances=all_distances,
        filtered_indices=filtered_indices,
        bbh_embeddings=bbh_embeddings,
        advbench_embeddings=advbench_embeddings,
        voicebench_files=bbh_audio_files,  # Keep name for compatibility
        advbench_files=advbench_audio_files
    )
    print(f"  Distance analysis: {base_analysis_path}")

    # Print some examples
    top10_label = "Top 10 safest (furthest) BBH samples from AdvBench" if select_safest else "Top 10 closest BBH samples to AdvBench"
    print(f"\n=== {top10_label} ===")
...
def main():
    parser = argparse.ArgumentParser(description="Filter BBH samples closest to AdvBench (acoustic)")
    parser.add_argument("--bbh_json", type=str,
                       default="data/bbh/bbh_full.json",
                       help="Path to BBH samples JSON file")
    ...
    parser.add_argument("--output_json", type=str,
                       default="data/bbh/bbh_closest_to_advbench.json",
    ...
    parser.add_argument("--cache_dir", type=str, default="data/bbh/embedding_cache",
