#!/bin/bash
# Step 0 (Text-Semantic): Filter VoiceBench samples by TEXT SEMANTIC embedding similarity
#
...
#
# Output is saved to data_semantic/ folder
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo ""
            echo "Filter VoiceBench by TEXT SEMANTIC embedding similarity to harmful prompts."
...
if [ -n "$NUM_SAMPLES" ]; then
    OUTPUT_JSON="data_semantic/voicebench/sd-qa/sd_qa_text_semantic_n${NUM_SAMPLES}.json"
    MODE_DESC="num_samples=${NUM_SAMPLES}"
elif [ -n "$PERCENTAGE" ]; then
    OUTPUT_JSON="data_semantic/voicebench/sd-qa/sd_qa_text_semantic_percentage_${PERCENTAGE}.json"
    MODE_DESC="percentage=${PERCENTAGE}%"
elif [ -n "$THRESHOLD" ]; then
    OUTPUT_JSON="data_semantic/voicebench/sd-qa/sd_qa_text_semantic_threshold_${THRESHOLD}.json"
...
echo "============================================"
echo "VoiceBench TEXT-SEMANTIC Filtering"
echo "============================================"
...
echo "============================================"
echo ""
echo "Next step: Run 1_prepare_filtered_dataset.sh with --filter_type text_semantic"
echo "  bash 1_prepare_filtered_dataset.sh --filter_type text_semantic --percentage $PERCENTAGE"
