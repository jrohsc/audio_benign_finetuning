#!/bin/bash
# Step 0 (Audio-Acoustic): Filter HeySQuAD samples by TRUE ACOUSTIC embedding distance
#
# This script filters HeySQuAD samples based on TRUE AUDIO ACOUSTIC characteristics.
...
CACHE_DIR="data_acoustic_heysquad/embedding_cache"
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo ""
            echo "Filter HeySQuAD by TRUE ACOUSTIC embedding similarity to harmful audio."
            echo "Uses Whisper-Large-V3 encoder ONLY (without AF3's projector)."
            echo "Results saved to data_acoustic_heysquad/ folder."
            echo ""
...
if [ -n "$NUM_SAMPLES" ]; then
    OUTPUT_JSON="data_acoustic_heysquad/heysquad_acoustic_n${NUM_SAMPLES}.json"
    MODE_DESC="num_samples=${NUM_SAMPLES}"
    FILTER_ARGS="--num_samples $NUM_SAMPLES"
elif [ -n "$PERCENTAGE" ]; then
    OUTPUT_JSON="data_acoustic_heysquad/heysquad_acoustic_percentage_${PERCENTAGE}.json"
    MODE_DESC="percentage=${PERCENTAGE}%"
    FILTER_ARGS="--percentage $PERCENTAGE"
elif [ -n "$THRESHOLD" ]; then
    OUTPUT_JSON="data_acoustic_heysquad/heysquad_acoustic_threshold_${THRESHOLD}.json"
...
echo "============================================"
echo "HeySQuAD TRUE ACOUSTIC Filtering"
echo "============================================"
...
# Run filtering (reuse spoken_squad script with heysquad paths)
...
echo "============================================"
echo ""
echo "Next step: Run 1_prepare_filtered_dataset.sh with appropriate options"
