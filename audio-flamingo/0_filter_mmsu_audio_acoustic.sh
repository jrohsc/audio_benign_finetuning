#!/bin/bash
# Step 0 (Audio-Acoustic): Filter MMSU samples by TRUE ACOUSTIC embedding distance
#
# This script filters MMSU (VoiceBench Multi-Subject Understanding) samples based on
# TRUE AUDIO ACOUSTIC characteristics.
...
CACHE_DIR="data_acoustic_mmsu/embedding_cache"
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "Results saved to data_acoustic_mmsu/ folder."
            echo ""
...
if [ -n "$NUM_SAMPLES" ]; then
    OUTPUT_JSON="data_acoustic_mmsu/mmsu_acoustic_n${NUM_SAMPLES}.json"
    MODE_DESC="num_samples=${NUM_SAMPLES}"
    FILTER_ARGS="--num_samples $NUM_SAMPLES"
elif [ -n "$PERCENTAGE" ]; then
    OUTPUT_JSON="data_acoustic_mmsu/mmsu_acoustic_percentage_${PERCENTAGE}.json"
    MODE_DESC="percentage=${PERCENTAGE}%"
    FILTER_ARGS="--percentage $PERCENTAGE"
elif [ -n "$THRESHOLD" ]; then
    OUTPUT_JSON="data_acoustic_mmsu/mmsu_acoustic_threshold_${THRESHOLD}.json"
...
echo "============================================"
echo "MMSU TRUE ACOUSTIC Filtering"
echo "============================================"
...
# Run filtering (reuse the generic filter script with mmsu paths)
python 0_filter_mmsu.py \
    --mmsu_json "$MMSU_JSON" \
