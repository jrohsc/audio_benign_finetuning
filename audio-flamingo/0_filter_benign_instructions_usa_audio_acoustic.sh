#!/bin/bash
# Filter Benign Instructions USA samples by TRUE ACOUSTIC embedding distance
#
# Uses Whisper-Large-V3 encoder ONLY (without AF3's multi-modal projector).
# Results saved to data_acoustic_benign_instructions_usa/ folder.
...
CACHE_DIR="data_acoustic_benign_instructions_usa/embedding_cache"
...
if [ -n "$NUM_SAMPLES" ]; then
    OUTPUT_JSON="data_acoustic_benign_instructions_usa/benign_instructions_usa_acoustic_n${NUM_SAMPLES}.json"
    MODE_DESC="num_samples=${NUM_SAMPLES}"
    FILTER_ARGS="--num_samples $NUM_SAMPLES"
elif [ -n "$PERCENTAGE" ]; then
    OUTPUT_JSON="data_acoustic_benign_instructions_usa/benign_instructions_usa_acoustic_percentage_${PERCENTAGE}.json"
    MODE_DESC="percentage=${PERCENTAGE}%"
    FILTER_ARGS="--percentage $PERCENTAGE"
elif [ -n "$THRESHOLD" ]; then
    OUTPUT_JSON="data_acoustic_benign_instructions_usa/benign_instructions_usa_acoustic_threshold_${THRESHOLD}.json"
...
echo "============================================"
echo "Benign Instructions USA TRUE ACOUSTIC Filtering"
echo "============================================"
...
python 0_filter_heysquad.py \
    ...
    --dataset_name "Benign Instructions USA" \
    $FILTER_ARGS \
