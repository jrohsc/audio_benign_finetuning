...
# Results saved to data_benign_instructions_accents/ folder.
...
CACHE_DIR="data_benign_instructions_accents/embedding_cache"
...
if [ -n "$NUM_SAMPLES" ]; then
    OUTPUT_JSON="data_benign_instructions_accents/benign_instructions_accents_audio_semantic_n${NUM_SAMPLES}.json"
    MODE_DESC="num_samples=${NUM_SAMPLES}"
    FILTER_ARGS="--num_samples $NUM_SAMPLES"
elif [ -n "$PERCENTAGE" ]; then
    OUTPUT_JSON="data_benign_instructions_accents/benign_instructions_accents_audio_semantic_percentage_${PERCENTAGE}.json"
    MODE_DESC="percentage=${PERCENTAGE}%"
    FILTER_ARGS="--percentage $PERCENTAGE"
elif [ -n "$THRESHOLD" ]; then
    OUTPUT_JSON="data_benign_instructions_accents/benign_instructions_accents_audio_semantic_threshold_${THRESHOLD}.json"
