#!/bin/bash
# Step 0 (Audio-Semantic): Filter LibriSpeech samples by AUDIO SEMANTIC embedding distance
#
# This script filters LibriSpeech samples based on how AF3 "understands" audio content.
# Uses Audio-Flamingo 3's full encoder pipeline (AudioFlamingo3Encoder + MultiModalProjector).
#
# THREE FILTER TYPES AVAILABLE:
#   - audio_acoustic (0_filter_librispeech_audio_acoustic.sh): TRUE acoustic features
#       Uses: Whisper-Large-V3 encoder only
#       Output: data_acoustic/librispeech/
#
#   - audio_semantic (this script): Audio content as understood by model
#       Uses: AF3's AudioFlamingo3Encoder + MultiModalProjector
#       Output: data/librispeech/
#
#   - text_semantic (0_filter_librispeech_text_semantic.sh): Text meaning similarity
#       Uses: sentence-transformers (all-MiniLM-L6-v2)
#       Output: data_semantic/librispeech/
...
LIBRISPEECH_JSON="data/librispeech/librispeech_full.json"
ADVBENCH_AUDIO_DIR="../harmful_data/advbench_gtts/en"
CACHE_DIR="data/librispeech/embedding_cache_af3"
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo ""
            echo "Filter LibriSpeech by AUDIO SEMANTIC embedding similarity to harmful audio."
            echo "Uses AF3's full encoder pipeline (AudioFlamingo3Encoder + MultiModalProjector)."
            echo "Results saved to data/librispeech/ folder."
            echo ""
            ...
            echo "  --librispeech_json PATH     LibriSpeech JSON file"
...
if [ -n "$NUM_SAMPLES" ]; then
    OUTPUT_JSON="data/librispeech/librispeech_audio_semantic_n${NUM_SAMPLES}.json"
    MODE_DESC="num_samples=${NUM_SAMPLES}"
    FILTER_ARGS="--num_samples $NUM_SAMPLES"
elif [ -n "$PERCENTAGE" ]; then
    OUTPUT_JSON="data/librispeech/librispeech_audio_semantic_percentage_${PERCENTAGE}.json"
    MODE_DESC="percentage=${PERCENTAGE}%"
    FILTER_ARGS="--percentage $PERCENTAGE"
elif [ -n "$THRESHOLD" ]; then
    OUTPUT_JSON="data/librispeech/librispeech_audio_semantic_threshold_${THRESHOLD}.json"
...
echo "============================================"
echo "LibriSpeech AUDIO-SEMANTIC Filtering"
echo "============================================"
...
echo "LibriSpeech JSON:  $LIBRISPEECH_JSON"
echo "AdvBench audio:    $ADVBENCH_AUDIO_DIR"
echo "Mode:              $MODE_DESC"
echo "Output:            $OUTPUT_JSON"
echo "============================================"

python 0_filter_librispeech_audio_semantic.py \
    --model_path "$MODEL_PATH" \
    --librispeech_json "$LIBRISPEECH_JSON" \
...
echo "============================================"
echo ""
echo "Next step: Run 1_prepare_filtered_dataset.sh with --benign_dataset librispeech --filter_type audio_semantic"
echo "  bash 1_prepare_filtered_dataset.sh --benign_dataset librispeech --filter_type audio_semantic --percentage $PERCENTAGE"
