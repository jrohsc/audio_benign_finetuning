#!/bin/bash
# Step 0 (Audio-Acoustic): Filter GammaCorpus Accents samples by TRUE ACOUSTIC embedding distance
#
# This script filters GammaCorpus Accents samples based on TRUE AUDIO ACOUSTIC characteristics.
...
CACHE_DIR="data_acoustic_gammacorpus_accents/embedding_cache"
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo ""
            echo "Filter GammaCorpus Accents by TRUE ACOUSTIC embedding similarity to harmful audio."
...
if [ -n "$NUM_SAMPLES" ]; then
    OUTPUT_JSON="data_acoustic_gammacorpus_accents/gammacorpus_accents_acoustic_n${NUM_SAMPLES}.json"
    MODE_DESC="num_samples=${NUM_SAMPLES}"
    FILTER_ARGS="--num_samples $NUM_SAMPLES"
elif [ -n "$PERCENTAGE" ]; then
    OUTPUT_JSON="data_acoustic_gammacorpus_accents/gammacorpus_accents_acoustic_percentage_${PERCENTAGE}.json"
    MODE_DESC="percentage=${PERCENTAGE}%"
    FILTER_ARGS="--percentage $PERCENTAGE"
elif [ -n "$THRESHOLD" ]; then
    OUTPUT_JSON="data_acoustic_gammacorpus_accents/gammacorpus_accents_acoustic_threshold_${THRESHOLD}.json"
...
echo "GammaCorpus Accents JSON: $GAMMACORPUS_JSON"
...
# Run filtering (reuse heysquad filter script with gammacorpus_accents paths)
...
echo "============================================"
echo ""
echo "Next step: Run 1_prepare_filtered_dataset.sh with --benign_dataset gammacorpus_accents --filter_type audio_acoustic"
