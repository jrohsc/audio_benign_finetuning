#!/usr/bin/env python3
"""
Filter LibriSpeech samples based on SEMANTIC (text) embedding distance to harmful prompts.

This script uses sentence-transformers to extract text embeddings that capture
the MEANING of the transcription content.

Based on 0_filter_voicebench_semantic.py but adapted for LibriSpeech dataset.
"""
...
def load_librispeech_json(json_path: str) -> Tuple[List[Dict], List[str]]:
    """Load LibriSpeech JSON and extract transcription texts."""
    print(f"Loading LibriSpeech data from {json_path}")
    ...
    for item in data:
        ...
        if not text and 'conversations' in item:
            # Fallback: get from conversations
...
def filter_by_semantic_embedding(
    ...
    cache_dir: str = "data/librispeech/embedding_cache_semantic",
    device: str = "cuda",
    ...
    """
    Filter LibriSpeech samples by semantic (text) embedding distance to harmful prompts.
    """
    ...
    librispeech_data, librispeech_texts = load_librispeech_json(librispeech_json)
    ...
    librispeech_cache = os.path.join(cache_dir, f"librispeech_semantic_{model_suffix}.npy")
    ...
    if os.path.exists(librispeech_cache):
        print(f"\nLoading cached LibriSpeech embeddings from {librispeech_cache}")
        librispeech_embeddings = np.load(librispeech_cache)
    else:
        print("\nExtracting LibriSpeech text embeddings...")
        librispeech_embeddings = extractor.extract_embeddings(librispeech_texts, desc="LibriSpeech")
        np.save(librispeech_cache, librispeech_embeddings)
        print(f"Cached to {librispeech_cache}")
    ...
    print(f"\nEmbedding shapes:")
    print(f"  LibriSpeech: {librispeech_embeddings.shape}")
    print(f"  Harmful: {harmful_embeddings.shape}")

    # Compute distances
    print("\nComputing cosine distances...")
    all_distances = compute_cosine_distance(librispeech_embeddings, harmful_embeddings)
    ...
    if effective_top_k is not None:
        ...
        print(f"  Kept {len(filtered_indices)} / {len(librispeech_data)} samples")
    else:
    ...
    print(f"  Selected: {len(filtered_indices)} / {len(librispeech_data)} ({100*len(filtered_indices)/len(librispeech_data):.1f}%)")

    # Create filtered dataset (same format as original)
    filtered_data = []
    for idx in filtered_indices:
        item = librispeech_data[idx].copy()
    ...
    voicebench_files = np.array([item.get('audio', '') for item in librispeech_data])
    ...
    for i in range(min(10, len(filtered_data))):
        ...
        transcription = librispeech_texts[filtered_indices[i]][:60]
...
def main():
    parser = argparse.ArgumentParser(description="Filter LibriSpeech by SEMANTIC text embedding similarity")
    parser.add_argument("--librispeech_json", type=str,
                        default="data/librispeech/librispeech_full.json",
                        help="Path to LibriSpeech JSON file")
    ...
    parser.add_argument("--cache_dir", type=str, default="data/librispeech/embedding_cache_semantic",
