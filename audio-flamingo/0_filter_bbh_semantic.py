#!/usr/bin/env python3
"""
Filter BBH (BIG-Bench Hard) samples based on SEMANTIC (text) embedding distance to harmful prompts.
...
DIFFERENCE FROM ACOUSTIC FILTERING (0_filter_bbh.py):
...
Usage:
    python 0_filter_bbh_semantic.py \
        --harmful_texts_csv ../harmful_data/advbench.csv \
        --percentage 10 \
        --output_json data_semantic_bbh/bbh_text_semantic_percentage_10.json
"""
...
def load_bbh_json(json_path: str) -> Tuple[List[Dict], List[str]]:
    """Load BBH JSON and extract question texts."""
    print(f"Loading BBH data from {json_path}")
...
def filter_by_semantic_embedding(
    ...
    cache_dir: str = "data_semantic_bbh/embedding_cache",
    device: str = "cuda",
    ...
    """
    Filter BBH samples by semantic (text) embedding distance to harmful prompts.
    """
    ...
    # Load BBH data
    bbh_data, bbh_texts = load_bbh_json(bbh_json)
    ...
    bbh_cache = os.path.join(cache_dir, f"bbh_semantic_{model_suffix}.npy")
    ...
    if os.path.exists(bbh_cache):
        print(f"\nLoading cached BBH embeddings from {bbh_cache}")
        bbh_embeddings = np.load(bbh_cache)
    else:
        print("\nExtracting BBH text embeddings...")
        bbh_embeddings = extractor.extract_embeddings(bbh_texts, desc="BBH")
        np.save(bbh_cache, bbh_embeddings)
        print(f"Cached to {bbh_cache}")
    ...
    print(f"\nEmbedding shapes:")
    print(f"  BBH: {bbh_embeddings.shape}")
    print(f"  Harmful: {harmful_embeddings.shape}")

    # Compute distances
    print("\nComputing cosine distances...")
    all_distances = compute_cosine_distance(bbh_embeddings, harmful_embeddings)
    ...
    if effective_top_k is not None:
        ...
        print(f"  Kept {len(filtered_indices)} / {len(bbh_data)} samples")
    else:
    ...
    print(f"  Selected: {len(filtered_indices)} / {len(bbh_data)} ({100*len(filtered_indices)/len(bbh_data):.1f}%)")

    # Create filtered dataset (same format as original)
    filtered_data = []
    for idx in filtered_indices:
        item = bbh_data[idx].copy()
    ...
    bbh_files = np.array([item.get('audio', '') for item in bbh_data])
    ...
    np.savez(
        ...
        voicebench_files=bbh_files,  # Required by 1_prepare_filtered_dataset.py
    ...
    for i in range(min(10, len(filtered_data))):
        ...
        question = bbh_texts[filtered_indices[i]][:60]
...
def main():
    parser = argparse.ArgumentParser(description="Filter BBH by SEMANTIC text embedding similarity")
    parser.add_argument("--bbh_json", type=str,
                        default="data/bbh/bbh_full.json",
                        help="Path to BBH JSON file")
    ...
    parser.add_argument("--cache_dir", type=str, default="data_semantic_bbh/embedding_cache",
