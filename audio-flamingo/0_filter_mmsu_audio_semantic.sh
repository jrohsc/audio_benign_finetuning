#!/bin/bash
# Step 0 (Audio-Semantic): Filter MMSU samples by AUDIO SEMANTIC embedding distance
#
# This script filters MMSU (VoiceBench Multi-Subject Understanding) samples based on
# how AF3 "understands" audio content.
...
MMSU_JSON="data/mmsu/mmsu_full.json"
ADVBENCH_AUDIO_DIR="../harmful_data/advbench_gtts/en"
CACHE_DIR="data/mmsu/embedding_cache_af3"
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo ""
            echo "Filter MMSU by AUDIO SEMANTIC embedding similarity to harmful audio."
            echo "Uses AF3's full encoder pipeline (AudioFlamingo3Encoder + MultiModalProjector)."
            echo "Results saved to data/mmsu/ folder."
            echo ""
            ...
            echo "  --mmsu_json PATH            MMSU JSON file"
...
if [ -n "$NUM_SAMPLES" ]; then
    OUTPUT_JSON="data/mmsu/mmsu_audio_semantic_n${NUM_SAMPLES}.json"
    MODE_DESC="num_samples=${NUM_SAMPLES}"
    FILTER_ARGS="--num_samples $NUM_SAMPLES"
elif [ -n "$PERCENTAGE" ]; then
    OUTPUT_JSON="data/mmsu/mmsu_audio_semantic_percentage_${PERCENTAGE}.json"
    MODE_DESC="percentage=${PERCENTAGE}%"
    FILTER_ARGS="--percentage $PERCENTAGE"
elif [ -n "$THRESHOLD" ]; then
    OUTPUT_JSON="data/mmsu/mmsu_audio_semantic_threshold_${THRESHOLD}.json"
...
echo "============================================"
echo "MMSU AUDIO-SEMANTIC Filtering"
echo "============================================"
...
echo "MMSU JSON:         $MMSU_JSON"
echo "AdvBench audio:    $ADVBENCH_AUDIO_DIR"
echo "Mode:              $MODE_DESC"
echo "Output:            $OUTPUT_JSON"
echo "============================================"

python 0_filter_mmsu_audio_semantic.py \
...
echo "============================================"
echo ""
echo "Next step: Run 1_prepare_filtered_dataset.sh with --benign_dataset mmsu --filter_type audio_semantic"
echo "  bash 1_prepare_filtered_dataset.sh --benign_dataset mmsu --filter_type audio_semantic --percentage $PERCENTAGE"
