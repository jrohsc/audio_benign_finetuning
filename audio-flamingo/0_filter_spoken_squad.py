#!/usr/bin/env python3
"""
Filter Spoken-SQuAD samples to find those CLOSEST in embedding space to AdvBench samples.
Uses Audio Flamingo 3's pretrained Whisper encoder or original Whisper to compute audio embeddings.

This is based on 0_filter_closest_to_advbench.py but adapted for Spoken-SQuAD dataset.
"""
...
def filter_spoken_squad_samples(
    ...
    """
    Filter Spoken-SQuAD samples to find those CLOSEST to AdvBench samples.
    Uses original Whisper-Large-V3 encoder (TRUE ACOUSTIC features).
    """
    # Load Spoken-SQuAD data
    print(f"Loading Spoken-SQuAD data from {spoken_squad_json}")
    with open(spoken_squad_json, 'r') as f:
        spoken_squad_data = json.load(f)

    # Get audio file paths
    json_dir = Path(spoken_squad_json).parent
    spoken_squad_audio_files = []
    for sample in spoken_squad_data:
        ...
        spoken_squad_audio_files.append(audio_path)

    print(f"Found {len(spoken_squad_audio_files)} Spoken-SQuAD samples")
    if spoken_squad_audio_files:
        print(f"First audio path: {spoken_squad_audio_files[0]}")
    ...
    if cache_dir:
        ...
        ss_cache = str(cache_dir / "spoken_squad_embeddings_whisper.npz")
        ab_cache = str(cache_dir / "advbench_en_embeddings_whisper.npz")

    # Compute embeddings using original Whisper
    print(f"\n=== Computing Spoken-SQuAD embeddings using original Whisper encoder ===")
    spoken_squad_embeddings = compute_whisper_embeddings_original(
        spoken_squad_audio_files,
        cache_path=ss_cache
    )
    ...
    min_distances, all_distances = compute_distances(spoken_squad_embeddings, advbench_embeddings, metric)

    # Print statistics
    print(f"\nDistance statistics (Spoken-SQuAD to AdvBench):")
    ...
    if effective_top_k is not None:
        ...
        if select_safest:
            ...
            filtered_indices = np.where(min_distances <= threshold)[0]
            print(f"\n=== Filtering with threshold <= {threshold} ===")
        print(f"  Kept {len(filtered_indices)} / {len(spoken_squad_data)} samples ({100*len(filtered_indices)/len(spoken_squad_data):.1f}%)")
    ...
    filtered_data = [spoken_squad_data[i] for i in filtered_indices]
    ...
    base_analysis_path = output_path.parent / "spoken_squad_closest_to_advbench_analysis.npz"
    np.savez(
        base_analysis_path,
        ...
        voicebench_files=spoken_squad_audio_files,  # Keep name for compatibility
        advbench_files=advbench_audio_files
    )
    print(f"  Distance analysis: {base_analysis_path}")

    # Print some examples
    top10_label = "Top 10 safest (furthest) Spoken-SQuAD samples from AdvBench" if select_safest else "Top 10 closest Spoken-SQuAD samples to AdvBench"
    print(f"\n=== {top10_label} ===")
...
def main():
    parser = argparse.ArgumentParser(description="Filter Spoken-SQuAD samples closest to AdvBench (acoustic)")
    parser.add_argument("--spoken_squad_json", type=str,
                       default="data/spoken_squad/spoken_squad_full.json",
                       help="Path to Spoken-SQuAD samples JSON file")
    ...
    parser.add_argument("--output_json", type=str,
                       default="data/spoken_squad/spoken_squad_closest_to_advbench.json",
    ...
    parser.add_argument("--cache_dir", type=str, default="data/spoken_squad/embedding_cache",
