...
def prepare_filtered_dataset(
    ...
    modality: str = "audio",  # "audio" or "text"
    file_prefix: str = "voicebench_filtered",
):
    """
    ...
    Args:
        ...
        modality: "audio" for audio+text dataset, "text" for text-only dataset
    """
    # Load analysis data
    ...
    # Try different key names for benign files (voicebench, mmsu, bbh, etc.)
    benign_file_keys = ['voicebench_files', 'mmsu_files', 'bbh_files', 'spoken_squad_files',
                        'librispeech_files', 'heysquad_files', 'benign_files']
    voicebench_files = None
    for key in benign_file_keys:
        if key in analysis:
            voicebench_files = analysis[key]
            print(f"  Found benign files under key: {key}")
            break
    if voicebench_files is None:
        raise KeyError(f"Could not find benign files in analysis. Available keys: {list(analysis.keys())}")
    ...
    output_json = output_dir / f"{file_prefix}_{filter_type}_{threshold_str}.json"
    ...
    hf_format_data = []
    text_format_data = []  # Text-only format for text BFT

    # Generic instruction to ensure model learns to extract question from audio
    generic_instruction = "Please answer the following question based on the audio."
    ...
    for sample in filtered_data:
        ...
        if len(conversations) >= 2:
            ...
            # Remove <audio> tag from user message to get the text question (for text-only format)
            question_text = user_msg.replace('<audio>', '').replace('\n', ' ').strip()

            # Audio+Text format (for audio finetuning)
            # Use generic instruction so model learns to extract question FROM audio
            hf_sample = {
                "id": sample.get('id', ''),
                "audio_path": sample['audio'],
                "conversations": [
                    {
                        ...
                        "content": [
                            ...
                            {"type": "text", "text": generic_instruction},
                        ]
                    },
                    {
                        "role": "assistant",
            ...
            # Text-only format (for text BFT) - keep original question for text-only baseline
            text_sample = {
                "id": sample.get('id', ''),
                "conversations": [
                    {
            ...
            text_format_data.append(text_sample)

    # Save audio format
    hf_output_json = output_dir / f"{file_prefix}_{filter_type}_{threshold_str}_hf.json"
    with open(hf_output_json, 'w') as f:
        json.dump(hf_format_data, f, indent=2)
    print(f"  HuggingFace format (audio): {hf_output_json}")

    # Save text-only format
    text_output_json = output_dir / f"{file_prefix}_{filter_type}_{threshold_str}_text.json"
    with open(text_output_json, 'w') as f:
        json.dump(text_format_data, f, indent=2)
    print(f"  Text-only format: {text_output_json}")
    ...
    return str(output_json), str(hf_output_json), str(text_output_json)
...
def main():
    ...
    parser.add_argument(
        "--file_prefix",
        type=str,
        default="voicebench_filtered",
        help="Prefix for output filenames (e.g., 'librispeech_filtered')"
    )
    parser.add_argument(
        "--skip_analysis",
        action="store_true",
        help="Skip analysis file and use pre-filtered JSON directly (for random filter)"
    )
    ...
    if args.skip_analysis:
        # For pre-filtered data (e.g., random filter), just convert to training format
        prepare_prefiltered_dataset(
...
def prepare_prefiltered_dataset(
    voicebench_json: str,
    output_dir: str,
    file_prefix: str = "voicebench_filtered",
):
    """
    Prepare pre-filtered dataset (e.g., from random filtering) for finetuning.
    No analysis file needed - data is already filtered.
    """
    output_dir = Path(output_dir)
    output_dir.mkdir(parents=True, exist_ok=True)

    # Load pre-filtered data
    print(f"Loading pre-filtered data from {voicebench_json}")
    with open(voicebench_json) as f:
    ...
    output_json = output_dir / f"{file_prefix}_random.json"
    with open(output_json, 'w') as f:
        json.dump(filtered_data, f, indent=2)
    print(f"\n=== Saved filtered dataset ===")
    print(f"  Output: {output_json}")
    ...
    # Save audio format
    hf_output_json = output_dir / f"{file_prefix}_random_hf.json"
    with open(hf_output_json, 'w') as f:
        json.dump(hf_format_data, f, indent=2)
    print(f"  HuggingFace format (audio): {hf_output_json}")

    # Save text-only format
    text_output_json = output_dir / f"{file_prefix}_random_text.json"
    with open(text_output_json, 'w') as f:
        json.dump(text_format_data, f, indent=2)
    print(f"  Text-only format: {text_output_json}")
    ...
    for i in range(min(3, len(filtered_data))):
        sample = filtered_data[i]
        print(f"\n{i+1}. Audio: {sample.get('audio', 'N/A')}")
        if 'conversations' in sample:
            q = sample['conversations'][0].get('value', '')[:80]
            a = sample['conversations'][1].get('value', '')[:80]
            print(f"   Q: {q}...")
