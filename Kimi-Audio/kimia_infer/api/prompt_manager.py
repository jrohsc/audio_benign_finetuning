...
class KimiAPromptManager:
    def __init__(self, model_path: str, kimia_token_offset: int, kimia_text_audiodelaytokens: int):
        ...
        # )
        whisper_model_path = os.path.join(model_path, "whisper-large-v3")
        if not os.path.exists(whisper_model_path):
            whisper_model_path = "openai/whisper-large-v3"
            logger.warning(f"Using HuggingFace whisper model: {whisper_model_path}")

        self.whisper_model = WhisperEncoder(
            whisper_model_path, mel_batch_size=20
        )
