...
Filter VoiceBench Noisy variant samples based on SEMANTIC embedding distance to harmful audio.
...
2. Whisper-Large-V3 Encoder -> Continuous ACOUSTIC features
   ...
   - See: 0_filter_voicebench_noise_acoustic.py for true acoustic filtering

Usage:
    python 0_filter_voicebench_noise_audio_semantic.py \
        --noise_type cafe \
        --harmful_dir /path/to/advbench_gtts/en \
        --percentage 50 \
        --output data_voicebench_cafe/voicebench_cafe_filtered.jsonl
"""
...
def center_embeddings(emb1: np.ndarray, emb2: np.ndarray):
...
def filter_and_save(
    ...
    # Print stats
    print(f"\n{'='*60}")
    print(f"SEMANTIC Distance Distribution (all {dataset_name} samples):")
...
def main():
    parser = argparse.ArgumentParser(
        description="Filter VoiceBench Noisy variants by AUDIO SEMANTIC embedding distance (WhisperVQEncoder)"
    )
    ...
    parser.add_argument("--center", action="store_true",
                        help="Center embeddings by subtracting global mean before computing distance. "
                             "CRITICAL for Kimi-Audio: raw embeddings have ~99.96%% of their norm "
    ...
    if args.output is None:
        args.output = f"data_{dataset_name}/{dataset_name}_filtered.jsonl"
    if args.cache_dir is None:
        args.cache_dir = f"embedding_cache_{dataset_name}"
    ...
    voicebench_cache = os.path.join(args.cache_dir, f"{dataset_name}_semantic_embeddings.npy")
    ...
    if os.path.exists(voicebench_cache):
        print(f"Loading cached {dataset_name} SEMANTIC embeddings from {voicebench_cache}")
        ...
        voicebench_embeddings = extractor.extract_embeddings_batch(
            voicebench_audio_paths, desc=f"{dataset_name} (Semantic)"
        )
    ...
    # Center embeddings (CRITICAL for Kimi-Audio)
    if args.center:
        ...
        # Save centered embeddings for reproducibility
        np.save(os.path.join(args.cache_dir, f"{dataset_name}_semantic_embeddings_centered.npy"), voicebench_embeddings)
