...
"""
=============================================================================
AUDIO-BASED SEMANTIC FILTERING - HeySQuAD
=============================================================================

Filter HeySQuAD samples based on SEMANTIC embedding distance to harmful audio.
...
Output: data_heysquad/ folder (for audio_semantic filter type)
...
def load_heysquad_data(jsonl_path: str, dataset_name: str = None) -> List[Dict]:
    """Load dataset from JSONL file."""
    if dataset_name is None:
        dataset_name = Path(jsonl_path).stem
    print(f"Loading {dataset_name} dataset from {jsonl_path}...")

    data = []
    with open(jsonl_path, "r") as f:
        for line in f:
            entry = json.loads(line.strip())
            # Extract audio path - try direct "audio" field first (heysquad_accents format)
            # then fall back to conversation format (Kimi-Audio native format)
...
def main():
    parser = argparse.ArgumentParser(
        description="Filter HeySQuAD by SEMANTIC embedding similarity to harmful audio"
    )
    parser.add_argument("--heysquad_jsonl", type=str, default="data_heysquad/heysquad_full.jsonl",
                        help="Path to HeySQuAD JSONL file")
    ...
    parser.add_argument("--output", type=str, default="data_heysquad/heysquad_filtered_semantic.jsonl",
                        help="Output JSONL file")
    parser.add_argument("--cache_dir", type=str, default="embedding_cache_semantic_heysquad",
    ...
    parser.add_argument("--dataset_name", type=str, default=None,
                        help="Dataset name for logging (auto-detected from jsonl path if not provided)")
    ...
    if dataset_name is None:
        dataset_name = Path(args.heysquad_jsonl).stem.replace("_full", "")

    # Initialize SEMANTIC embedding extractor
    extractor = KimiSemanticEmbeddingExtractor(device=args.device)

    # Load data
    data = load_heysquad_data(args.heysquad_jsonl, dataset_name)
    ...
    benign_cache = os.path.join(args.cache_dir, f"{dataset_name}_semantic_embeddings.npy")
    harmful_cache = os.path.join(args.cache_dir, f"harmful_semantic_embeddings_{Path(args.harmful_dir).name}.npy")

    if os.path.exists(benign_cache):
        print(f"Loading cached {dataset_name} embeddings from {benign_cache}")
        embeddings = np.load(benign_cache)
    else:
        print(f"Extracting {dataset_name} SEMANTIC embeddings...")
        embeddings = extractor.extract_embeddings_batch(audio_paths, desc=dataset_name)
        np.save(benign_cache, embeddings)
        print(f"Cached embeddings to {benign_cache}")
    ...
    if args.center:
        ...
        np.save(os.path.join(args.cache_dir, f"{dataset_name}_semantic_embeddings_centered.npy"), embeddings)
