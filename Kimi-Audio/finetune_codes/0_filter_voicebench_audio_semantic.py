...
Filter VoiceBench samples based on SEMANTIC embedding distance to harmful audio.
...
2. Whisper-Large-V3 Encoder -> Continuous ACOUSTIC features
   ...
   - See: 0_filter_voicebench_acoustic.py for true acoustic filtering

This script extracts SEMANTIC embeddings from AUDIO files using WhisperVQEncoder.
Compare with:
- 0_filter_voicebench_text_semantic.py: SEMANTIC from TEXT (sentence-transformers)
- 0_filter_voicebench_acoustic.py: ACOUSTIC from AUDIO (Whisper-Large-V3)

Pipeline:
1. Extracts semantic embeddings from VoiceBench audio files
2. Extracts semantic embeddings from harmful audio files
3. Computes pairwise cosine similarity
4. Filters samples that are semantically close to harmful samples
5. Saves filtered data for finetuning
...
def filter_and_save(
    ...
    Args:
        ...
        select_safest: If True, select samples FURTHEST from harmful (safest benign samples)
    ...
    # Sanity check: ensure data and embeddings have matching lengths
    if len(voicebench_data) != len(voicebench_embeddings):
        print(f"WARNING: Length mismatch! voicebench_data={len(voicebench_data)}, embeddings={len(voicebench_embeddings)}")
        print("This can happen if embeddings were cached with different data.")
        print("Consider deleting the embedding cache and re-running.")
        # Truncate both to the minimum length
        min_len = min(len(voicebench_data), len(voicebench_embeddings))
        voicebench_data = voicebench_data[:min_len]
        voicebench_embeddings = voicebench_embeddings[:min_len]
        harmful_embeddings = harmful_embeddings  # Keep as-is
    ...
    if use_distance:
        ...
        # For distance: lower = closer to harmful, higher = further (safer)
        if select_safest:
            sort_ascending = False  # Sort descending so largest distances (safest) come first
        else:
            sort_ascending = True  # Sort ascending so smallest distances (closest to harmful) come first
    else:
        ...
        # For similarity: higher = closer to harmful, lower = further (safer)
        if select_safest:
            sort_ascending = True  # Sort ascending so lowest similarities (safest) come first
        else:
            sort_ascending = False  # Sort descending so highest similarities (closest to harmful) come first

    # Priority: num_samples > percentage > top_k > threshold
    original_percentage = percentage
    original_num_samples = num_samples

    # Use the number of samples we have metric values for (should match voicebench_data after truncation)
    ...
    if num_samples is not None:
        ...
        top_k = int(n_samples * percentage / 100)
        top_k = max(1, top_k)  # Ensure at least 1 sample
        print(f"\n=== Converting {percentage}% to top_k={top_k} samples (out of {n_samples}) ===")
    ...
    if top_k is not None:
        ...
        if select_safest:
            print(f"\n=== Keeping top {top_k} SAFEST samples (furthest from harmful) ===")
        else:
        ...
        if use_distance:
            if select_safest:
                # Select safest: keep samples with distance >= threshold (furthest from harmful)
                mask = metric_values >= threshold
                filter_desc = f">= {threshold}"
            else:
                # Default: keep samples with distance <= threshold (closest to harmful)
                mask = metric_values <= threshold
                filter_desc = f"<= {threshold}"
        else:
            if select_safest:
                # Select safest: keep samples with similarity <= threshold (furthest from harmful)
                mask = metric_values <= threshold
                filter_desc = f"<= {threshold}"
            else:
                # Default: keep samples with similarity >= threshold (closest to harmful)
                mask = metric_values >= threshold
                filter_desc = f">= {threshold}"
    ...
    print(f"  Total VoiceBench samples: {n_samples}")
