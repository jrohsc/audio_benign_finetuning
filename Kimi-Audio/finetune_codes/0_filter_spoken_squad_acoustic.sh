#!/bin/bash
# Filter Spoken-SQuAD samples by TRUE ACOUSTIC embedding distance
#
...
# Output directory
OUTPUT_DIR="data_acoustic_spoken_squad"
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo ""
            echo "Filter Spoken-SQuAD by TRUE ACOUSTIC embedding similarity."
...
OUTPUT="${OUTPUT_DIR}/spoken_squad_filtered_acoustic_${OUTPUT_SUFFIX}.jsonl"
...
# Build command
CMD="CUDA_VISIBLE_DEVICES=0 python 0_filter_spoken_squad_acoustic.py"
CMD="$CMD --spoken_squad_jsonl \"$SPOKEN_SQUAD_JSONL\""
CMD="$CMD --harmful_dir \"$HARMFUL_DIR\""
CMD="$CMD --output \"$OUTPUT\""
CMD="$CMD --cache_dir \"embedding_cache_acoustic_spoken_squad\""
CMD="$CMD --device cuda"
