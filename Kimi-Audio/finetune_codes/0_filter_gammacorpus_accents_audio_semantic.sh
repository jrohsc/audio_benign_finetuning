#!/bin/bash
# =============================================================================
# AUDIO-BASED SEMANTIC FILTERING - GammaCorpus Accents
# =============================================================================
#
# Filter GammaCorpus Accents samples by SEMANTIC embedding similarity to harmful audio.
# Uses GLM-4 Voice Tokenizer (WhisperVQEncoder) - captures WHAT is being said.
#
# Output: data_gammacorpus_accents/ folder (for audio_semantic filter type)
#
...
OUTPUT_DIR="data_gammacorpus_accents"
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo ""
            echo "Filter GammaCorpus Accents by SEMANTIC embedding similarity to harmful audio."
            echo "Uses GLM-4 Voice Tokenizer (WhisperVQEncoder) - captures linguistic content."
            echo ""
            echo "Output: data_gammacorpus_accents/ folder"
            echo ""
            echo "Options:"
            ...
            echo "  --output_dir PATH       Output directory (default: data_gammacorpus_accents)"
...
OUTPUT="${OUTPUT_DIR}/gammacorpus_accents_filtered_semantic_${OUTPUT_SUFFIX}.jsonl"

# Create output directory
mkdir -p "$OUTPUT_DIR"

echo "============================================"
echo "GammaCorpus Accents SEMANTIC Filtering"
echo "============================================"
...
# Build command (reuse heysquad semantic filter script - it handles the "audio" field format)
CMD="CUDA_VISIBLE_DEVICES=0 python 0_filter_heysquad_audio_semantic.py"
CMD="$CMD --harmful_dir \"$HARMFUL_DIR\""
CMD="$CMD --output \"$OUTPUT\""
CMD="$CMD --cache_dir \"embedding_cache_semantic_gammacorpus_accents\""
CMD="$CMD --device cuda"
CMD="$CMD --heysquad_jsonl \"data/gammacorpus_accents/gammacorpus_accents_full.jsonl\""
...
echo "============================================"
echo ""
echo "Next step: Run 1_extract_semantic_tokens.sh with --benign_dataset gammacorpus_accents --filter_type audio_semantic"
echo "  bash 1_extract_semantic_tokens.sh --benign_dataset gammacorpus_accents --filter_type audio_semantic --percentage $PERCENTAGE"
