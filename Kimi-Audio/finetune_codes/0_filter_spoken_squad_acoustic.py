#!/usr/bin/env python3
"""
Filter Spoken-SQuAD samples by TRUE ACOUSTIC embedding distance to harmful audio.
Uses Whisper-Large-V3 encoder for acoustic features.

Output: data_acoustic_spoken_squad/ folder
"""
...
def main():
    parser = argparse.ArgumentParser(
        description="Filter Spoken-SQuAD by TRUE ACOUSTIC embedding distance"
    )
    ...
    parser.add_argument("--output", type=str, default="data_acoustic_spoken_squad/spoken_squad_filtered.jsonl",
                        help="Output JSONL file")
    parser.add_argument("--cache_dir", type=str, default="embedding_cache_acoustic_spoken_squad",
    ...
    spoken_squad_cache = os.path.join(args.cache_dir, "spoken_squad_acoustic_embeddings.npy")
    ...
    if os.path.exists(spoken_squad_cache):
        print(f"Loading cached Spoken-SQuAD embeddings from {spoken_squad_cache}")
        embeddings = np.load(spoken_squad_cache)
    else:
        print("Extracting Spoken-SQuAD ACOUSTIC embeddings...")
        embeddings = extractor.extract_embeddings_batch(audio_paths, desc="Spoken-SQuAD")
        np.save(spoken_squad_cache, embeddings)
        print(f"Cached embeddings to {spoken_squad_cache}")
