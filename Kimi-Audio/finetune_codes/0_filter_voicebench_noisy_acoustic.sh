...
# =============================================================================
#
# Filter VoiceBench Noisy samples by TRUE ACOUSTIC embedding similarity to harmful audio.
...
#
# Output: data_acoustic_voicebench_noisy/ folder
...
OUTPUT_DIR="data_acoustic_voicebench_noisy"
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "  --output_dir PATH       Output directory (default: data_acoustic_voicebench_noisy)"
...
OUTPUT="${OUTPUT_DIR}/voicebench_noisy_filtered_${HARMFUL_SOURCE}_acoustic_${OUTPUT_SUFFIX}.jsonl"
...
# Build command - use the voicebench_noisy filter script
CMD="CUDA_VISIBLE_DEVICES=0 python 0_filter_voicebench_noisy_acoustic.py"
CMD="$CMD --harmful_dir \"$HARMFUL_DIR\""
CMD="$CMD --output \"$OUTPUT\""
CMD="$CMD --cache_dir \"embedding_cache_acoustic_voicebench_noisy\""
CMD="$CMD --device cuda"
...
echo "============================================"
echo ""
echo "Next step: Run 1_extract_semantic_tokens.sh with --filter_type acoustic --benign_dataset voicebench_noisy"
