...
"""
=============================================================================
AUDIO-BASED ACOUSTIC FILTERING (TRUE ACOUSTIC FEATURES) - VoiceBench Noisy
=============================================================================

Filter VoiceBench Noisy samples based on TRUE ACOUSTIC embedding distance to harmful audio.
...
Usage:
    python 0_filter_voicebench_noisy_acoustic.py \
        --harmful_dir /path/to/advbench_gtts/en \
        --percentage 50 \
        --output data_acoustic_voicebench_noisy/voicebench_noisy_filtered_advbench_50.jsonl
"""
...
def filter_and_save(
    ...
    # Print stats
    print(f"\n{'='*60}")
    print(f"ACOUSTIC Distance Distribution (all VoiceBench Noisy samples):")
...
def main():
    parser = argparse.ArgumentParser(
        description="Filter VoiceBench Noisy by TRUE ACOUSTIC embedding distance (Whisper-Large-V3)"
    )
    ...
    parser.add_argument("--output", type=str, default="data_acoustic_voicebench_noisy/voicebench_noisy_filtered.jsonl",
                        help="Output JSONL file")
    parser.add_argument("--cache_dir", type=str, default="embedding_cache_acoustic_voicebench_noisy",
    ...
    voicebench_cache = os.path.join(args.cache_dir, "voicebench_noisy_acoustic_embeddings.npy")
    ...
    if os.path.exists(voicebench_cache):
        print(f"Loading cached VoiceBench Noisy ACOUSTIC embeddings from {voicebench_cache}")
        voicebench_embeddings = np.load(voicebench_cache)
    else:
        print("Extracting VoiceBench Noisy ACOUSTIC embeddings...")
        voicebench_embeddings = extractor.extract_embeddings_batch(
            voicebench_audio_paths, desc="VoiceBench Noisy (Acoustic)"
        )
