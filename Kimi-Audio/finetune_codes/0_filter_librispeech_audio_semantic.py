...
"""
=============================================================================
AUDIO-BASED SEMANTIC FILTERING - LibriSpeech
=============================================================================

Filter LibriSpeech samples based on SEMANTIC embedding distance to harmful audio.
...
Output: data_librispeech/ folder (for audio_semantic filter type)
...
def load_librispeech_data(jsonl_path: str) -> List[Dict]:
    """Load LibriSpeech dataset from JSONL file."""
    print(f"Loading LibriSpeech dataset from {jsonl_path}...")
...
def main():
    parser = argparse.ArgumentParser(
        description="Filter LibriSpeech by SEMANTIC embedding similarity to harmful audio"
    )
    parser.add_argument("--librispeech_jsonl", type=str, default="data_librispeech/librispeech_full.jsonl",
                        help="Path to LibriSpeech JSONL file")
    ...
    parser.add_argument("--output", type=str, default="data_librispeech/librispeech_filtered_semantic.jsonl",
                        help="Output JSONL file")
    parser.add_argument("--cache_dir", type=str, default="embedding_cache_semantic_librispeech",
    ...
    # Load data
    data = load_librispeech_data(args.librispeech_jsonl)
    ...
    librispeech_cache = os.path.join(args.cache_dir, "librispeech_semantic_embeddings.npy")
    ...
    if os.path.exists(librispeech_cache):
        print(f"Loading cached LibriSpeech embeddings from {librispeech_cache}")
        embeddings = np.load(librispeech_cache)
    else:
        print("Extracting LibriSpeech SEMANTIC embeddings...")
        embeddings = extractor.extract_embeddings_batch(audio_paths, desc="LibriSpeech")
        np.save(librispeech_cache, embeddings)
        print(f"Cached embeddings to {librispeech_cache}")
    ...
    if args.center:
        ...
        np.save(os.path.join(args.cache_dir, "librispeech_semantic_embeddings_centered.npy"), embeddings)
